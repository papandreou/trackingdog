...
async function renderSnippetWithContext({ sourceAsset, line, column }) {
  await sourceAsset.loadAsync();
  const sourceLines = sourceAsset.text.split(/\n\r?|\r\n?/);
...
(async () => {
  for (let i = 0; i < nonOptionArgs.length; i += 1) {
    ...
    try {
      ...
      let { url, line, column, sourceAsset } = await trackingDog({
        root,
        ...generatedLocation
      });

      if (/^file:/.test(url)) {
        url = pathModule.relative(
          process.cwd(),
      ...
      if (context && sourceAsset) {
        try {
          const magicPen = await renderSnippetWithContext({
            sourceAsset,
            line,
            column
          });
