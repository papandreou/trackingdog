...
async function trackingDog({ root, url, line, column, assetGraph }) {
  if (!assetGraph) {
    assetGraph = new AssetGraph({ root });
  }

  let [asset] = assetGraph.findAssets({ url });

  if (!asset) {
    asset = assetGraph.addAsset(url);
  }
